...
no_fraud = len(train)-train.fraud.sum()
fraud = train.fraud.sum()

weight_for_0 = (1 / no_fraud)*(len(train))/2.0 
weight_for_1 = (1 / fraud)*(len(train))/2.0
...
for bs in [2,4,8]:
    for ep in [2,4,8]:
        ...
        train_history_last = model.fit(train_input_last, train_labels,validation_data=(test_input_last, test_labels),epochs=ep,callbacks=[checkpoint_last],batch_size=bs,class_weight=class_weight)
