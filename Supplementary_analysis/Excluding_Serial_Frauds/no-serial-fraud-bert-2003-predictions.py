...
def remove_serial_fraud(df):
    train0 = df[df.fraud == 0]
    train1 = df[df.fraud == 1]
    
    train1 = train1.sort_values(by=['year'])
    train1 = train1.drop_duplicates(subset='cik', keep="first")
...
train = remove_serial_fraud(train)
print(train.shape, test.shape, train.fraud.sum(), test.fraud.sum())
...
test.to_csv("test_2003_nsf.csv",index = False)