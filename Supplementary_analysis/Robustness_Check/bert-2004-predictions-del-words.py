...
train['nostop_text'] = train['text'].apply(lambda x: remove_stopwords(x))
...
print("-"*10,"Most frequent 30 words for train year", train.year.min(), "to", train.year.max(),"-"*10)
print(counts.sum().sort_values(ascending=False)[:30])
counts.sum().sort_values(ascending=False)[:30].plot.bar()
del_list = counts.sum().sort_values(ascending=False)[:30].index.values.tolist()
print(del_list)

for j in del_list:
    test['text'] = test['text'].str.replace(j,'wxyz',regex = True)
...
test.to_csv("test_2004_del_words.csv",index = False)